...
- 以下情况不适合使用HDFS：1）低延迟访问数据，2）大量的小文件，3）并发写，任意位置文件修改
- HDFS的块比磁盘的块大（一般为512KB)，目的是为了减小寻址开销
- 可靠性和带宽的权衡结果:如果客户端在集群上，在与客户端相同的节点上放置第一个副本;如果客户端不在集群上，则随机放置第一个副本;第二个副本被放置在与第一个不同的随机选择的机架上;第三个副本被放置在与第二个相同的机架上
- HDFS提供一个方法来强制所有的缓存与数据节点同步:在文件系统数据输出流使用sync()方法,在sync()返回成功后，HDFS能保证文件中直至写入的最后的数据对所有新的读取者而言，都是可见且一致的
...
- One of the strengths of HBase over a relational database is that you don’t have to specify all the columns up front
...
- Strictly speaking, an RDBMS is a database that follows Codd’s 12 rules. Typical RDBMSs are fixed-schema, row-oriented databases with ACID properties and a sophisticated SQL query engine. The emphasis is on strong consistency, referential integrity, abstrac‐ tion from the physical layer, and complex queries through the SQL language. You can easily create secondary indexes; perform complex inner and outer joins; and count, sum, sort, group, and page your data across a number of tables, rows, and columns.
